...
-- Question: How many rows does the result contain?
-- Answer: 20 rows (5 employees × 4 departments)
...
-- Answer: 4 rows. Tom Brown is not included because his dept_id is NULL and doesn’t match any department.
...
-- Answer: USING removes one duplicate column (dept_id), so the result has one less column.
...
-- Answer: Tom Brown appears with NULL values for department columns, because he has no matching dept_id.

SELECT emp_name, dept_id, dept_name, location
FROM employees
...
-- Answer:
-- NULL on the left side → departments with no employees (e.g., Marketing)
-- NULL on the right side → employees without departments (e.g., Tom Brown)
...
-- Answer:
-- Query 1 (ON clause): The filter applies before the join, so all employees appear,
-- but only departments in Building A are matched.
-- Query 2 (WHERE clause): The filter applies after the join, so employees without
-- departments in Building A are excluded completely.
...
-- Answer: No difference, because INNER JOIN returns only matching rows.
-- Filtering in ON or WHERE clause gives the same result for INNER JOIN.
